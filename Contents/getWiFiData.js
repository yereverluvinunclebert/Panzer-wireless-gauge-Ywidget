/*
    ...
    Copyright © 2010-2018 Harry Whitfield
...
function getAirportNetworks(data) {
    ...
    for (i = 0; i < dicts.length; i += 1) {
        ...
        delete networks[i].noise;
        networks[i].channel = parseInt(item[3], 10);
        networks[i].locked = item[secIndex] !== "NONE";
        networks[i].security = (item[secIndex + 1] !== undefined)
        	? item[secIndex] + " " + item[secIndex + 1]
        	: item[secIndex];
    }
    return networks;
}
