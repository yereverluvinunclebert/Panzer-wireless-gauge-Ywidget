...
/*property
    MouseWheelPref, actionSwitchPref, airport, altKey, available, busy, charAt,
    clockSize, ConnectServer, createObject, ctrlKey, data, defaultValue, devicePref, devices,
    duration, ease, endAngle, floor, hOffset, hRegistrationPoint, height, i,
    ibps, interval, isNaN, kEaseOut, keyCode, length, maxLength, maxSpeedPref,
    milliseconds, minLength, minSpeedPref, name, network, networkswitch, noise,
    obps, onKeyDown, onMouseDown, onMouseUp, onMouseWheel,
    onPreferencesChanged, onTimerFired, onWakeFromSleep,
    onWillChangePreferences, onload, opacity, option, optionValue, platform,
    powered, random, reset, rotation, round, sampleIntervalPref, scrollDelta,
    select, setTick, signal, size, soundPref, src, srcHeight, srcWidth, start,
    startAngle, startTime, ticking, ticks, toFixed, tooltip, traffic, type,
    vOffset, vRegistrationPoint, value, visible, width, wirelessSwitch
*/
...
//  if (preferences.WMIenabled.value === "enabled" && system.platform === "windows") {  // ****
    ...
    if (preferences.wirelessSwitch.value === "on") {
        ...
        if (systemAirport.available && systemAirport.powered) {
            ...
            wirelessPercentText.data = String("   " + wirelessText).slice(-3); // returns " 23" (with one space)
            wirelessPercentTextArea.data = wirelessPercentText;

            noiseLevel = systemAirport.noise || "-99"

            background.tooltip = "   Wireless signal " + "\n" + "   " + ("Network: ") + systemAirport.network + "\n" + "   " + ("Noise level: ") + (systemAirport.noise || "-99") + "db \n" + "   " + ("Signal level : ") + wirelessStrength + "%";
            ...
            networkScaleText.data = String("   " + percentageBandwidth).slice(-3); // returns " 23" (with one space)
            networkScaleTextArea.data = networkScaleText.data;
            ...
            wirelessPercentTextArea.data = " 00";
...
switchFacesButton.onMouseDown = function (event) {
    if (event.altKey) {
      if (preferences.WMIenabled.value === "enabled") {                                 // ****
         getDevices(true);      // get network adapters
         deviceMenu.select();
      }                                                                             // ****
    }
};
...
//  traffic = NETSTAT.traffic(devName);             // **** was this meant to be global
    ...
    pointer.rotation = 0.01 * value * 251 + 30; // was - 120;
}
//=====================
//End function
//=====================
...
//===============================================================
// these functions are called when the widget prefs are (to be) changed
//===============================================================
...
widget.onPreferencesChanged = function () {
    changePrefs();  // startup is called in changeprefs
    //startup();
};
//=====================
//End function
...
background.onMultiClick = function(event) {
	...
	if (event.ctrlKey) {
        ...
        if (preferences.imageCmdPref.value === "" && system.platform === "macintosh") {
        	preferences.imageCmdPref.value = "/Applications/Utilities/Network Utility.app";
        }
        if (preferences.imageCmdPref.value === "" && system.platform === "windows") {
        	preferences.imageCmdPref.value = "%SystemRoot%/system32/ncpa.cpl";
        }
